...
import net.yaly.viewpagerdoubleindicator.R;


public class ViewPagerDoubleIndicator extends HorizontalScrollView {
    ...
    private ColorStateList mTabTextColorUnSelected = null;
    ...
    private boolean isPaddingMiddle = false;//是否居中显示他不
    ...
    private boolean isHorizontalSplit = false;//是否水平平分显示控件

    private int mScrollOffset;
    private int mLastScrollX = 0;

    private int mTabBackgroundResId = R.drawable.vpdi_background_tab;//按下效果
    ...
    public ViewPagerDoubleIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        // get system attrs for container获得我们所定义的自定义样式属性
        ...
        // get custom attrs for tabs and container 获取attr中参数
        ...
        mIndicatorHeight = a.getDimensionPixelSize(R.styleable.ViewPagerDoubleIndicator_vpdiIndicatorHeight, mIndicatorHeight);
        mUnderlineColor = a.getColor(R.styleable.ViewPagerDoubleIndicator_vpdiUnderlineColor, mUnderlineColor);
        mUnderlineHeight = a.getDimensionPixelSize(R.styleable.ViewPagerDoubleIndicator_vpdiUnderlineHeight, mUnderlineHeight);
        mDividerColor = a.getColor(R.styleable.ViewPagerDoubleIndicator_vpdiDividerColor, mDividerColor);
        mDividerWidth = a.getDimensionPixelSize(R.styleable.ViewPagerDoubleIndicator_vpdiDividerWidth, mDividerWidth);
        mDividerPadding = a.getDimensionPixelSize(R.styleable.ViewPagerDoubleIndicator_vpdiDividerPadding, mDividerPadding);
        isExpandTabs = a.getBoolean(R.styleable.ViewPagerDoubleIndicator_vpdiShouldExpand, isExpandTabs);
        mScrollOffset = a.getDimensionPixelSize(R.styleable.ViewPagerDoubleIndicator_vpdiScrollOffset, mScrollOffset);
        isPaddingMiddle = a.getBoolean(R.styleable.ViewPagerDoubleIndicator_vpdiPaddingMiddle, isPaddingMiddle);
        isHorizontalSplit = a.getBoolean(R.styleable.ViewPagerDoubleIndicator_vpdiHorizontalSplit, isHorizontalSplit);
        mTabPadding = a.getDimensionPixelSize(R.styleable.ViewPagerDoubleIndicator_vpdiTabPaddingLeftRight, mTabPadding);
        mTabBackgroundResId = a.getResourceId(R.styleable.ViewPagerDoubleIndicator_vpdiTabBackground, mTabBackgroundResId);
        mTabTextSize = a.getDimensionPixelSize(R.styleable.ViewPagerDoubleIndicator_vpdiTabTextSize, mTabTextSize);
        mTabTextColor = a.hasValue(R.styleable.ViewPagerDoubleIndicator_vpdiTabTextColor) ? a.getColorStateList(R.styleable.ViewPagerDoubleIndicator_vpdiTabTextColor) : null;
        mTabTextColorUnSelected = a.hasValue(R.styleable.ViewPagerDoubleIndicator_vpdiTabTextColorUnSelected)?a.getColorStateList(R.styleable.ViewPagerDoubleIndicator_vpdiTabTextColorUnSelected):null;
        mTabTextTypefaceStyle = a.getInt(R.styleable.ViewPagerDoubleIndicator_vpdiTabTextStyle, mTabTextTypefaceStyle);
        isTabTextAllCaps = a.getBoolean(R.styleable.ViewPagerDoubleIndicator_vpdiTabTextAllCaps, isTabTextAllCaps);
        int tabTextAlpha = a.getInt(R.styleable.ViewPagerDoubleIndicator_vpdiTabTextAlpha, DEF_VALUE_TAB_TEXT_ALPHA);
        ...
        //设置子View的layout参数 TODO
        if (isHorizontalSplit) {
            mTabLayoutParams = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT, 1f);//子view平分宽度
        } else {
            mTabLayoutParams = isExpandTabs ? new LinearLayout.LayoutParams(0, LayoutParams.MATCH_PARENT, 1.0f) : new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);//子view平分宽度
        }
    }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (isCustomTabs) {
                ...
                tabView = LayoutInflater.from(getContext()).inflate(R.layout.vpdi_tab, this, false);
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // draw indicator line TODO  画底部的线
        if (mIndicatorHeight > 0) {
            ...
            //画顶部的指示器
            canvas.drawRect(lines.first + mPaddingLeft, 0, lines.second - mPaddingLeft, mIndicatorHeight, mRectPaint);
            //            LogUtil.i("画线："+"顶"+(height - mIndicatorHeight)+"底"+height+"左"+lines.first + mPaddingLeft+"右"+(lines.second + mPaddingLeft));
        }
    }
    ...
    public void setIsHorizontalSplit(boolean isHorizontalSplit) {
    ...
    public void setBaseLineColor(int baseLineColor) {
        //        try {
        //            this.mBaseLineColor = getResources().getColor(resId);
        //        }catch (Exception e){
        //            this.mBaseLineColor = Color.BLUE;
        //        }
        this.mBaseLineColor = baseLineColor;
        invalidate();
    }
